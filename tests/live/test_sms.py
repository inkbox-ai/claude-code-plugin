...
# lets the carrier test forge a valid delivery-failure webhook. Exported under
# a dedicated name so it doesn't collide with other suites' skip gates.
...
# listing a conversation containing such rows can crash SDK clients whose
# SmsDeliveryStatus enum predates that status — the carrier test's
# conversation-id lookup must run against a clean history.
#
# Loop evidence comes from the gateway log (the bridge's wake-up lines),
...
def _assert_internal_block_surfaced(log_offset: int) -> None:
    ...
    A synchronous 422 reaches the agent by one of two legitimate paths, and
    which one a real model takes is nondeterministic:

    * The agent emits a normal reply → the bridge routes it through
      ``send_to_contact`` → the delivery-failure loop wakes the agent
      ("Woke agent about failed outbound sms", stage=send_rejected).
    * The agent calls the ``inkbox_send_sms`` tool directly → the 422 comes
      back inline as the tool result and is logged
      ("inkbox_send_sms rejected: message_blocked_spam_filter ...").

    Both feed the block rule back to the agent; assert either, so the test
    doesn't hinge on the model's send-path choice.
    """
    log = _gateway_log_since(log_offset)
...
def test_sms_retry_after_carrier_delivery_failure(sms):
    ...
    how a real delivery would arrive through the tunnel. The bridge must wake
    the agent, and the agent must send a real follow-up SMS to the remote.
    """
    aut, remote = sms["aut"], sms["remote"]
...
def test_sms_retry_after_internal_spam_block(sms):
    ...
    bridge must surface that block to the agent, which is the loop engaging:
    either a delivery-failure wake-up (the agent emitted a normal reply routed
    through send_to_contact) or an inline tool rejection (the agent called
    inkbox_send_sms directly and got the 422 back as its tool result). Both
    feed the ``emoji_overload`` rule to the agent to act on.
    ...
    exhausting its send budget, is model behaviour — not a property of the
    retry loop. Loop mechanics (cap, budget, recovery) are covered
    deterministically by tests/test_delivery_failure_retry.py, and end-to-end
    recovery by test_sms_retry_after_carrier_delivery_failure.
