...
def _setup_signing_key(api_key: str, base_url: str, Inkbox: Any) -> None:
    ...
    if not prompt_yes_no("  Generate a new signing key now?", True):
        print_info("  Skipping; bridge will accept unsigned webhooks.")
...
def _wait_for_imessage_first_message(client: Any, identity: Any, handle: str) -> None:
    ...
    try:
        ...
        print_info("  Rerun `inkbox-claude setup` later to finish connecting.")
        return
    ...
    welcome = (
        f"You're connected! This is your iMessage channel to your Claude Code "
    ...
    try:
        ...
        pass
    print_info("  Start the bridge (`inkbox-claude run`) and keep chatting there.")
...
def _self_signup_flow(base_url: str, Inkbox: Any, InkboxAPIError: Any) -> tuple[Any | None, str, bool]:
    ...
    note = "Setting up a Claude Code agent on Inkbox."
    human_email = ""
    handle = ""
...
def _mint_agent_scoped_key(client: Any, identity: Any, InkboxAPIError: Any) -> str | None:
    ...
    try:
        created = client.api_keys.create(
            label=f"Claude Code bridge - {identity.agent_handle}",
            description=(
...
def _configure_project_dir() -> None:
    """Ask which directory Claude Code should work in and persist it.
    ...
    print_info("  Claude Code reads, searches, and edits files in this directory")
    print_info("  when it answers you. Point it at the repo you want it to work on.")

    current = _env("CLAUDE_PROJECT_DIR") or os.getcwd()
    chosen = prompt("  Directory Claude Code should work in", current).strip() or current
    chosen = str(Path(chosen).expanduser())
    if not os.path.isdir(chosen):
        print_warning(f"  {chosen} is not a directory yet — create it before running the bridge.")
    _save("CLAUDE_PROJECT_DIR", chosen)
...
def interactive_setup() -> None:
    ...
    Returns:
        None
    """
    print_header("Inkbox + Claude Code")
    print_info("Give your Claude Code agent its own Inkbox identity — mailbox,")
