...
**Delivery failures.** Outbound messages can silently fail — a carrier filters an SMS, an iMessage is declined, an email bounces. Inkbox reports these asynchronously, and the bridge subscribes to the hard-failure events (`text.delivery_failed`, `imessage.delivery_failed`, `message.bounced`/`message.failed`) so they actually arrive. When one does, the bridge correlates it back to the original send — by the message id it recorded when it sent, falling back to the webhook's contact/thread — and wakes that contact's session to tell Claude *which* message didn't land and *why*. The recovery reply is delivered **on the same channel/thread by default**, so Claude can just rewrite the message to retry; to reach you another way it uses its Inkbox tools (a different channel, or a call) and replies `[SILENT]` so nothing is re-sent on the dead channel. `text.delivery_unconfirmed` is treated as telemetry and never wakes the agent. Repeat webhooks for the same message are de-duplicated, a recovery send that itself fails is not recovered again, and per-contact recovery is capped — so it can't loop.
